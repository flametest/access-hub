...
> **状态：v6（M1–M5 已实现，M6 已实现 2026-09-02）**
...
M6 修订（7 元组 + 显式优先级 + ABAC 条件）：

```ini
[request_definition]  r = sub, dom, obj, act
[policy_definition]   p = priority, sub, dom, obj, act, eft, cond
[role_definition]     g = _, _, _
[policy_effect]       e = priority(p.eft) || deny
[matchers]            m = (r.sub == p.sub || p.sub == "*" || g(r.sub, p.sub, r.dom) || g(r.sub, p.sub, "*"))
                          && (r.dom == p.dom || p.dom == "*")
                          && (r.obj == p.obj || p.obj == "*")
                          && (r.act == p.act || p.act == "*")
                          && (p.cond == "" || abacEval(p.cond, r.sub, r.dom, r.obj, r.act))
```

- **优先级阶梯（数值越小越先匹配、先到先得）**：super_admin 通配 1 < grant deny 20 < grant allow 30 < custom rule（默认 40，行级可调）< role deny 45 < role allow 50 < client 60；eft 恒显式（allow/deny）
- **abacEval 自定义函数（expr-lang）**：typed env `{sub, dom, obj, act, roles, now}`（dom 为裸 app key，roles 为该域 + 通配域去并、去 role: 前缀）；编译期拒绝未知标识符/函数（沙箱保证，取代设计初稿的 100ms 超时方案），recover 兜底，编译缓存按表达式复用；求值失败 fail-close
- **custom_rules 业务表**（per-app）：`(app_id, name)` 唯一、expr/effect/priority/status；管理端 CRUD + dry-run 端点；加载期编译失败的行跳过（Warn），CRUD 期由 ValidateExpr 拦截
- **menus 改为 enforcer 驱动**：节点可见 ⇔ 对该账号 Enforce 通过（deny 感知、custom rule 生效）；`/me/permissions` 维持 DB 推导（仅 allow 侧，按钮提示用途，服务端判定以 authz/check 为准）
...
原型缺口已于 M4–M6 全部补齐：注册页、忘记密码流程页、2FA 设置向导（/identity/2fa）、管理后台全部界面（/admin/*）。原型中 "Sign in to an existing account / Unlink" 屏随绑定模型简化废弃（绑定在创建时自动发生），子账号直登发生在各业务 app 自己的登录页（原型未含）。另有两处文案笔误已在门户实现中修正：`Compay ID`→`Company ID`；演示数据 `Altas`→`Atlas`。
...
- **M5 社交登录（已实现，2026-09-01；identities 表 + 四 provider（google/microsoft/facebook 经 x/oauth2，apple ES256 client_secret + form_post + JWKS 验证）；一次性 state/login_code；verified email 自动合并 + allowAutoRegister 自动注册；mode=link 绑定 + 最后登录方式守卫；/auth/social/complete 尊重 2FA 挑战 + pending_invitations 提示；门户四家按钮 + /social/complete + 关联账户管理卡；接入指南 docs/social-login-setup.md。实施决策：实施决策：① provider 凭证走 server-config.yaml 全局配置（org 级自带凭证后置）② 回调成功后签发一次性 login_code（kv，5min）→ 门户 `POST /auth/social/complete` 换取正式 tokens，token 不进 URL ③ 门户提供全部四家按钮；社交登录遵守 2FA 挑战 ④ verified email 自动合并维持既定决策）：**Google/Microsoft/Facebook（x/oauth2）、Apple（ES256 + form_post）；identities 表；verified email 自动合并；社交登录后按邮箱匹配待接受邀请/同名子账号的提示流程（对应原型 Discover 屏的落地形态）；org 邀请链接直达。2026-09-24 nOAuth 加固：邮箱自动合并收紧为仅限线上显式验证声明（Google/Apple email_verified、Microsoft email_verified 或 id_token xms_edov 绑定到同一地址）；Facebook（平台合同隐含验证）与 Entra 管理员可改写的 bare mail/UPN 一律不自动合并，只能登录态显式 link 或自动注册新号
- **M6 自定义规则（已实现，2026-09-02；casbin 模型升级为 7 元组 priority/eft/cond + expr-lang ABAC 沙箱（编译缓存 + recover 兜底，取代 100ms 超时方案）；优先级阶梯 deny/priority 全链路生效（loader/增量同步/业务表 effect 列启用）；`PUT /roles/:id/resources` 支持 `items:[{resource_id,effect}]`（`resource_ids` 兼容）、grants 支持 effect；`/me/menus` 改 enforcer 驱动（deny 感知）；管理端 custom-rules CRUD + dry-run test（`admin:customrule:read|manage`，org_admin 自动绑定）+ 审计报表 `GET /admin/audit-logs/summary?days=`（GROUP BY，days 夹取 1..90）；集成测试覆盖 role deny 翻转 authz/menus、deny grant 压过 role allow、custom rule 按条件授权、审计汇总形态）：**custom_rules（expr-lang ABAC）挂 matcher；effect=deny + priority；审计报表 + 管理后台 UI（最终实现：并入门户的 /admin/* 路由，见下方 M6 决策）
