...
CREATE UNIQUE INDEX IF NOT EXISTS uq_custom_rules_app_name ON custom_rules (app_id, name) WHERE deleted_at IS NULL;
CREATE INDEX IF NOT EXISTS idx_custom_rules_app ON custom_rules (app_id) WHERE deleted_at IS NULL;
